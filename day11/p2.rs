...
fn main() {
    ...
    for i in 0..galaxies.len() {
        ...
        for other in others {
            ...
            const EXP:Num = 1_000_000 - 1;
            
            // normal dist
            ans += (g.0 - other.0).abs() + (g.1 - other.1).abs();
            // additional dist
            ans += EXP * empty_rows[g.0.min(other.0) as usize..g.0.max(other.0) as usize]
                .iter()
                .filter(|&b| *b)
                .count() as Num;
            ans += EXP * empty_cols[g.1.min(other.1) as usize..g.1.max(other.1) as usize]
                .iter()
                .filter(|&b| *b)
                .count() as Num;
        }
